...
> ⚠️ **2026-08-13：このファイルは古い。**
> 下の数字は「左レール15個・右12個」だった頃のもので、いまは **左6・右5**。
> `sounds.ts` は削除され、設定画面・同意画面・撮影モード選択も実装済み。
> **CSS の在り処についても、ここの記述は当てにならない**
> （実際に効くのは `src/setup.css` だけ。`src/App.css` は読み込まれていない）。
> → 最新は [`tinycube-handoff-2026-08-13.md`](./tinycube-handoff-2026-08-13.md)
...
- **フレームはすでに96枚ある。**`public/frames/` の WebP と `src/frames.ts`。
  CMCUBE と同じ絵の中央を黒抜きして WebP にしたもの。**新規に作らなくても動く**
- **比率の問題は `anchor` で解決済み。**9スライスではない。
  `wide`=16:9 ／ `full`=9:16 ／ `top`・`bottom`=端だけの飾りで縦横どちらでも成立。
  形が合わない組み合わせも**「欠けてでも全部使いたい」という伊波さんの判断**で
  選べるようにしてあり、欠けるものはタイルに印が出る
- **9スライスは「追加の道」であって、前提ではない。**
  `recorder.ts` の `drawFrame` は `if (slice)` で分岐し、`slice` を持たない
  既存96枚は今までどおり `anchor` で描かれる。既存の動作は壊れていない。
  ⚠️ **描き下ろしのイラスト枠に9スライスを使ってはいけない**（中央を引き伸ばすと
  絵が歪む）。9スライスが正しいのは、角と辺が幾何学的に分かれている枠だけ
- **`backdrop-filter`（枠の後ろの映像をぼかす）は、PNG でも SVG でも再現できない。**
  canvas に直接描いたときだけ可能。
  → 2026-08-12 調査済み（ボタンくん）：v0 の CSS フレーム10種に
  `backdrop-filter` は1つも無かった
- **`backdrop-filter`（枠の後ろの映像をぼかす）は、PNG でも SVG でも再現できない。**
  canvas に直接描いたときだけ可能。
  → **2026-08-12 調査済み（ボタンくん）：v0 の CSS フレーム10種に

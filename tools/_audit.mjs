...
const run = async (lang) => {
  ...
  // ⚠️ **50枚超という条件は 2026-08-31 に古くなった。**
  //    鍵つき53枚が「推し色キラリフレームセット」の畳める段へ移ったので、
  //    無料の一覧は 縦45枚 / 横40枚 になる（前は 67枚）。
  //    段を開けば 縦98 / 横93 で、どちらの向きでも有料は53枚そろう（実測）。
  //    ここは**無料の一覧だけ**を数えるので、35枚あれば足りる
  ok(`フレーム${fr.n}枚・鍵${fr.鍵}・見本${fr.見本}`, fr.n>35 && fr.見本>0);
  // 指の横取り

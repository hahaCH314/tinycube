...
for (const file of files) {
  ...
  const code = `(async () => {
    ...
    // **マゼンタで塗ってある絵は、その色を抜くだけでよい。**
    // 「どこを抜くか」を絵の側で指定してもらう形（2026-08-14、伊波さんが
    // ホラー用に「ホラーマゼンダ.png」を用意）。黒の濃さで迷わずに済み、
    // 枠の中の暗い装飾や、内側へ伸びる枝を巻き込む心配も無い。
    // 中心がマゼンタなら、この絵はその方式だと判断する
    {
      const ci = (((H/2)|0)*W + ((W/2)|0)) * 4;
      const r0 = d[ci], g0 = d[ci+1], b0 = d[ci+2];
      const isMagenta = r0 > 180 && g0 < 80 && b0 > 140;
      if (isMagenta) {
        // **縁のぼかしまで消すこと。** 判定を狭くすると、抜いたまわりに
        // マゼンタの線がくっきり残る（2026-08-14、実測で確認）。
        // 「赤と青が高く、緑が低い」＝マゼンタ寄り、を広めに拾い、
        // 中途半端に混ざった画素は薄めて境目をなじませる
        let n = 0;
        for (let i = 0; i < N; i++) {
          const o = i*4;
          if (d[o+3] <= 8) continue;
          const r = d[o], gg = d[o+1], bb = d[o+2];
          // マゼンタらしさ：赤と青のうち低いほうが、緑よりどれだけ高いか
          const m = Math.min(r, bb) - gg;
          if (m > 60) { d[o+3] = 0; n++; continue; }          // しっかりマゼンタ
          if (m > 18) {                                        // 縁のぼかし
            // 緑に寄っているぶんだけ残す。境目が硬くならない
            const keep = (m - 18) / (60 - 18);
            d[o+3] = Math.round(d[o+3] * (1 - keep));
            // 残った色からマゼンタ味を抜いて、ピンクの縁が浮かないようにする
            d[o] = Math.min(r, gg + 20);
            d[o+2] = Math.min(bb, gg + 20);
            n++;
          }
        }
        if (n > 0) {
          g.putImageData(im, 0, 0);
          return JSON.stringify({ cleared: n, W, H, th: 'マゼンタ',
            holes: [{ x:0, y:0, w:0, h:0, size:n }], png: c.toDataURL('image/png') });
        }
      }
    }
